			...
			if (body.indexOf('list_share') != -1 || body.indexOf('method=list') != -1) {
				...
				$.post(req, (err, resp, data) => {
					try {
						...
						if(data.indexOf("password")!=-1) { $.msg($.name, "", "此文件夹需要密码！");}
						...
						const result = isRootFolder ? { total: 0, offset: 0, shares: files} : { total: 0,offset: 0,files: files};
						obj = {
							success: true,
							data: result
						};
			...
			if (body.indexOf('list_share') != -1 || body.indexOf('method=list') != -1) {
				...
				$.post(req, (err, resp, data) => {
					try {
						...
						if(playurls.length>0){ /* consider start */
...
async function AlistDownLoad() {
	return new Promise(resolve => {
		...
		if(encodeURIComponent(fileid).indexOf(encodeURIComponent(item.name))!=-1){				
				var target = item.url;
				if(!target) { target = host + '/d' + fileid;}
